...
Many of you should have gone through the Python prep seciton by now and should be ready to start the R prep section. This section is designed to help you get familiar with the R programming language.

Many of the concepts you will have learnt for Python will be applicable to R, but there are some key differences in syntax and functionality.

## R markdown

While the most basic way to run R code is to use the R console, we will be using R markdown files to run our code. R markdown files are a great way to combine code, output, and text in a single document.

Rmd or R markdown files can be edited using your IDE or intergrated development environment of your choice, but it can also be edited using a simple text editor like notepad. The advantage of using Rmd over a simple R script is that you can include text, code, and output in a single document and have it be rendered or knitted into a variety of formats like HTML, PDF, or Word.

Simillar to Jupiter notebooks, R markdown files are divided into cells. Each cell can contain either text or code. You can run the code in a cell by pressing the run button or by using the keyboard shortcut `Ctrl + Enter`.
...
For our purposes, this means that unlike python where we can access methods and properties of objects, in R we will be working with functions and dataframes.
...
# Python code
x = [1, 2, 3, 4, 5]
x.append(6) # This dot notation is used to access the append method of the list object
print(x)
```
...
4. Control flow in R
5. Functions in R
6. Variable naming conventions in R
...
1. **Numeric** - This is the most common data type in R and is used to store numbers. Numeric data types can be integers or floats.
...
# Lists
z <- list(1, "Hello, World!", TRUE)
print(z)

# Data Frames
# This is the closest analogue to a pandas dataframe in Python
a <- data.frame(x = c(1, 2, 3, 4, 5), y = c("a", "b", "c", "d", "e"))
print(a)
```
...
y <- 10
z <- x + y
print(sprintf("The sum of %d and %d is %d", x, y, z))
```
